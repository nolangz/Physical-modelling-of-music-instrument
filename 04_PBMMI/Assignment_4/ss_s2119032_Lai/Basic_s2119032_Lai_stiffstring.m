...
hmin = sqrt(1/2*(c^2*k^2+sqrt(c^4*k^4+16*K^2*k^2))); % minimal step
N = floor(L/(hmin));                                 % Number of step
h = L/N;                                             % calculate step again
...
assert(T>0||r>0||E>0||rho>0||T60>0||Tf>0,'error! Please check your input parameter')
...
f = zeros(1,Nf);                        % initialise force signal
...
spdiags_1 = spdiags([e -4*e 6*e -4*e e], -2:2, N-1,N-1);  % sparse matrix
spdiags_1(1,1) = 5;
spdiags_1(N-1,N-1) = 5;
Dxxxx = (1/h^4)*spdiags_1;                                % Dxxxx sparse matrix
B = 1/(1+sig*k)*(2 * I_mtx + c^2*k^2*Dxx -K^2*k^2*Dxxxx); % coefficient of u1
C = -(-1 + sig*k)/(1+sig*k)*I_mtx;                        % coefficient of u2
